...
def find_view_files(base_path: str) -> List[str]:
    ...
    perspective_path = os.path.join(base_path, "com.inductiveautomation.perspective", "views")
...
def analyze_codebase(codebase_name: str, base_path: str, schema: dict) -> Dict[str, Any]:
            ...
            for component in components:
                ...
                if error_info:
                    ...
                    if "is not of type" in error_msg:
                        ...
                        property_type_usage[path_str][actual_type].add(str(failing_value)[:100])  # Truncate long values
                else:
                    valid_components += 1
    ...
    success_rate = (valid_components / total_components * 100) if total_components > 0 else 0
    ...
    return {
        ...
        'property_type_usage': {k: {t: list(v) for t, v in types.items()} for k, types in property_type_usage.items()}
    }
...
def compare_codebases(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if all_error_patterns:
        ...
        sorted_patterns = sorted(all_error_patterns.items(), key=lambda x: len(x[1]), reverse=True)
        ...
        for pattern, occurrences in sorted_patterns[:10]:
            ...
            # Count by codebase
            codebase_counts = Counter(occ.get('file', '').split('/')[-4] for occ in occurrences)
            
            print(f"   📍 {path}: {actual_type} → {expected_types} ({len(occurrences)} total)")
    ...
    return {
        ...
        'overall_success_rate': total_valid/total_components*100 if total_components > 0 else 0,
        'all_component_types': sorted(all_component_types),
        'aggregated_error_patterns': dict(all_error_patterns)
    }

def suggest_schema_improvements_from_multi_codebase(aggregated_results: Dict[str, Any]) -> List[Dict]:
    ...
    suggestions.sort(key=lambda x: (x['priority'] == 'HIGH', x['occurrences']), reverse=True)
...
def main():
    ...
    if suggestions:
        ...
        print(f"🔥 {len(high_priority)} HIGH PRIORITY suggestions need immediate attention")
