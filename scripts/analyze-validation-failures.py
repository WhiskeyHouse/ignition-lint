...
def analyze_type_mismatches():
    ...
    print(f"   Success rate: {((total_components - failing_components) / total_components * 100):.1f}%")
    ...
    for pattern, occurrences in sorted_patterns[:20]:  # Top 20 patterns
        ...
        # Show sample values
        sample_values = list(set(str(occ['value'])[:50] for occ in occurrences[:5]))
        print(f"   Sample values: {sample_values}")
        
        # Show component types affected
        comp_types = set(occ['component_type'] for occ in occurrences)
        ...
        if len(type_usage) > 1:
            ...
            if 'int' in current_types and 'str' in current_types:
                suggestions.append({
                    ...
                    'solution': f'Change type to ["number", "string"]',
                    'examples': {t: list(vals)[:3] for t, vals in type_usage.items()}
                })
            ...
            if 'dict' in current_types and 'str' in current_types:
                suggestions.append({
                    ...
                    'solution': f'Change type to ["object", "string"]',
                    'examples': {t: list(vals)[:3] for t, vals in type_usage.items()}
                })
            
            if 'list' in current_types and ('dict' in current_types or 'str' in current_types):
                suggestions.append({
                    'property': prop_path,
...
def generate_schema_patch(suggestions: List[Dict]) -> Dict:
    ...
    for suggestion in suggestions:
        ...
        patches[prop_path] = {
            ...
            "description": f"Flexible type based on production usage analysis"
        }
...
def main():
    ...
    results = {
        'analysis_summary': {
            ...
            'properties_with_multiple_types': len([p for p, t in property_type_usage.items() if len(t) > 1]),
