...
def apply_final_fixes():
    ...
    if 'text' in props:
        ...
        if 'number' not in current_type:
            ...
            fixes_applied.append("props.text: ['string', 'null'] → ['string', 'null', 'number']")
    
    # 3. Fix meta.visible to allow null values
    meta_props = schema['properties']['meta']['properties']
    if 'visible' in meta_props:
        current_type = meta_props['visible'].get('type', ['boolean', 'string', 'number'])
        if 'null' not in current_type:
            meta_props['visible']['type'] = ['boolean', 'string', 'number', 'null']
            fixes_applied.append("meta.visible: ['boolean', 'string', 'number'] → ['boolean', 'string', 'number', 'null']")
...
def main():
    ...
    try:
        ...
        print(f"🧪 Run multi-codebase analysis to verify improvements")
