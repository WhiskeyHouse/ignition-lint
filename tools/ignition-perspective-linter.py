...
class IgnitionPerspectiveLinter:
    ...
    def _validate_jython_script(self, script_content: str, prop_name: str, context: str, file_path: str, component_path: str, comp_type: str):
        """Validate Jython script for whitespace, syntax, and best practices."""
        if not script_content or not script_content.strip():
            return
        
        # Check indentation patterns
        self._check_jython_indentation(script_content, prop_name, context, file_path, component_path, comp_type)
        
        # Check syntax
        self._check_jython_syntax(script_content, prop_name, context, file_path, component_path, comp_type)
        
        # Check Ignition-specific patterns
        self._check_ignition_best_practices(script_content, prop_name, context, file_path, component_path, comp_type)
    
    def _check_jython_indentation(self, script: str, prop_name: str, context: str, file_path: str, component_path: str, comp_type: str):
        """Check Jython indentation patterns including Ignition requirements."""
        lines = script.split('\n')
        
        # Track indentation issues
        mixed_indent_lines = []
        space_only_lines = []
        tab_lines = []
        ...
        if non_indented_lines:
            self.issues.append(LintIssue(
                ...
                message=f"Ignition requirement violation in {context}: Lines {non_indented_lines[:5]} have no indentation",
                file_path=file_path,
                component_path=f"{component_path}.{prop_name}",
                component_type=comp_type,
                line_suggestion="ALL lines in Ignition inline scripts must have at least 1 tab or 4 spaces indentation"
            ))
        
        # Report other indentation issues
        ...
        if space_only_lines and tab_lines:
            self.issues.append(LintIssue(
                severity=LintSeverity.INFO,
                code="JYTHON_INCONSISTENT_INDENT_STYLE",
                message=f"Mixed indentation styles in {context}: {len(tab_lines)} tab lines, {len(space_only_lines)} space lines",
                file_path=file_path,
                component_path=f"{component_path}.{prop_name}",
                component_type=comp_type,
                line_suggestion="Use tabs consistently (matches production pattern: 70.9% of scripts use tabs)"
            ))
    
    def _check_jython_syntax(self, script: str, prop_name: str, context: str, file_path: str, component_path: str, comp_type: str):
        """Check Jython script syntax using Python AST.
        
        Normalizes Ignition-style indentation (where ALL lines must be indented) 
        before Python syntax validation to avoid false positives.
        """
        try:
            # Normalize indentation for Python syntax validation
            # Ignition requires ALL lines to be indented, but Python AST expects 
            # the first line to have zero indentation for module-level statements
            normalized_script = textwrap.dedent(script)
            
            # Only validate if we have actual content after normalization
            if normalized_script.strip():
                ast.parse(normalized_script)
        except SyntaxError as e:
            # Check if this is a real syntax error or just Ignition formatting
            lines = script.split('\n')
            non_empty_lines = [line for line in lines if line.strip()]
            
            # If all non-empty lines are properly indented for Ignition, this might be a valid script
            all_lines_indented = all(line.startswith('\t') or line.startswith('    ') for line in non_empty_lines)
            
            if all_lines_indented:
                # This is likely a valid Ignition script with proper indentation
                # Try to validate the normalized version more carefully
                try:
                    normalized = textwrap.dedent(script)
                    if normalized.strip():
                        ast.parse(normalized)
                        # If we get here, it's actually valid - don't report an error
                        return
                except:
                    pass  # Fall through to report the original error
            ...
            self.issues.append(LintIssue(
                ...
                message=f"Could not parse {context} script: {str(e)}",
                file_path=file_path,
                component_path=f"{component_path}.{prop_name}",
                component_type=comp_type,
                line_suggestion="Check script for fundamental syntax issues"
            ))
    
    def _check_ignition_best_practices(self, script: str, prop_name: str, context: str, file_path: str, component_path: str, comp_type: str):
        """Check for Ignition-specific best practices."""
        lines = script.split('\n')
        
        # Check for hardcoded localhost (found in 47.2% of problem scripts)
        ...
        # Check for print vs system.perspective.print (found in production analysis)
        ...
        # Check for HTTP calls without exception handling (security best practice)
        if 'httpClient()' in script or 'httpPost' in script or 'httpGet' in script:
            if 'try:' not in script or 'except' not in script:
                self.issues.append(LintIssue(
                    ...
                    component_path=f"{component_path}.{prop_name}",
                    component_type=comp_type,
                    line_suggestion="Wrap HTTP calls in try/except blocks (found in 87% of production scripts)"
                ))
        
        # Check for common function usage patterns (based on empirical analysis)
        common_functions = ['getChild', 'getSibling', 'sendMessage', 'closePopup']
        missing_error_handling = []
        
        for func in common_functions:
            if func in script and 'try:' not in script:
                missing_error_handling.append(func)
        
        if missing_error_handling:
            self.issues.append(LintIssue(
                severity=LintSeverity.INFO,
                code="JYTHON_RECOMMEND_ERROR_HANDLING",
                message=f"Consider error handling for {', '.join(missing_error_handling)} in {context}",
                file_path=file_path,
                component_path=f"{component_path}.{prop_name}",
                component_type=comp_type,
                line_suggestion="Add error handling for component navigation functions"
            ))
